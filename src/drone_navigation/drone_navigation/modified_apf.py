...
import time
def save_failed_run():
    save_path = "plots/mppi/varying_sampled/run_results_mppi_varying_sampled.txt"
    try:
...
class ModifiedAPF(Node):
    def __init__(self):
        super().__init__('modified_apf_node')
    ...
    def sampling(self, ka, kp, d0):
        sigma_ka = 0.5
        sigma_kp = 0.5
        sigma_d0 = 0.5

        # Sample new values for ka, kp, and d0
        new_ka = abs(random.gauss(ka, sigma_ka))

        # Keep sampling for kp until it is above 0.5
        new_kp = abs(random.gauss(kp, sigma_kp))
        while new_kp <= 0.5:
            new_kp = abs(random.gauss(kp, sigma_kp))

        # Keep sampling for d0 until it is above 0.5
        new_d0 = abs(random.gauss(d0, sigma_d0))
        while new_d0 <= 0.5:
            new_d0 = abs(random.gauss(d0, sigma_d0))

        return new_ka, new_kp, new_d0

    # Optimization function to generate trajectories for sampled parameters
    def optimization(self):
        trajectories = []  # List to store generated trajectories
        objective_values = []  # List to store objective values for each trajectory
        weights = []  # List to store weights based on objective values

        # Initialize arrays to store sampled parameters
        sampled_k_att_list = []  # List to store sampled k_att values
        sampled_k_rep_list = []  # List to store sampled k_rep values
        sampled_d0_list = []     # List to store sampled d0 values

        # Generate initial trajectory with existing parameters
        initial_trajectory = self.gradient_descent_trajectory_Ut(
        ...
        for _ in range(self.sampling_count):
            ...
            #sampled_k_att, sampled_k_rep, sampled_d0 = self.sampling(self.k_att, self.k_rep, self.d0)
            sampled_k_att, sampled_k_rep, sampled_d0 = self.sampling(self.k_att, self.k_rep, self.d0)

            # Store sampled parameters in local arrays
            sampled_k_att_list.append(sampled_k_att)
            sampled_k_rep_list.append(sampled_k_rep)
            sampled_d0_list.append(sampled_d0)
            

            # Generate trajectory with the sampled parameters
            trajectory = self.gradient_descent_trajectory_Ut(
                obstacles=self.obstacles,
                d0=sampled_d0,
                k_att=sampled_k_att,
                k_rep=sampled_k_rep
            ...
            trajectories.append(trajectory)  # Store the trajectory

            # Call the objective function with the calculated trajectory
            objective_value = self.objective_function(trajectory,trajectories)
            objective_values.append(objective_value)
            l = 20
            # Calculate weight (replace this with your specific function)
            weight =  math.exp(-objective_value/l)
            weights.append(weight)

            self.get_logger().info(f'Sampled parameters: k_att={sampled_k_att}, k_rep={sampled_k_rep}, d0={sampled_d0}, Objective Value={objective_value}, Weight={weight}')

        # Now calculate the optimal trajectory
        optimal_trajectory = []

        for step in range(len(initial_trajectory)):
            initial_point = initial_trajectory[step]
            new_points = [trajectories[i][step] for i in range(len(trajectories))]  # Get the trajectory points for this step
            distances = [np.linalg.norm(initial_point - np.array(new_point)) for new_point in new_points]  # Calculate distances

            # Calculate the weighted sum and optimal trajectory point
            if sum(weights) > 0:  # Avoid division by zero
                weighted_sum = sum(weight * distance for weight, distance in zip(weights, distances))
                optimal_point = initial_point + (weighted_sum / sum(weights))
            else:
                optimal_point = initial_point  # Fallback if no weights

            optimal_trajectory.append(optimal_point)
        final_objective_value = self.objective_function(optimal_trajectory,trajectories)
        self.get_logger().info(f'final objective value : {final_objective_value}')

        # Find the trajectory closest to the optimal trajectory
        optimal_trajectory_array = np.array(optimal_trajectory)
        closest_index = np.argmin([
            np.sum(np.linalg.norm(optimal_trajectory_array - np.array(trajectory), axis=1))
            for trajectory in trajectories
        ])
        #closest_index = np.argmin([abs(objective_value - final_objective_value) for objective_value in objective_values])


        # Output corresponding sampled values
        closest_k_att = sampled_k_att_list[closest_index]
        closest_k_rep = sampled_k_rep_list[closest_index]
        closest_d0 = sampled_d0_list[closest_index]
        

        closest_trajectory = trajectories[closest_index]  # Get the closest trajectory
        self.k_att_history.append(closest_k_att)
        self.k_rep_history.append(closest_k_rep)
        self.d0_history.append(closest_d0)
        self.time_steps.append(len(self.k_att_history)) 
        self.plot_parameters()

        self.get_logger().info(f'Closest trajectory found at index {closest_index}: k_att={closest_k_att}, k_rep={closest_k_rep}, d0={closest_d0}')

        # Return initial trajectory, all generated trajectories, optimal trajectory, and closest trajectory
        return initial_trajectory, trajectories, optimal_trajectory, closest_trajectory # Return initial and optimal trajectories

    # Modify the start_trajectory_computation to call optimization
    def plot_parameters(self):
        self.ax11.clear()  # Clear the previous plot

    # Plot k_att over time
        self.ax11.plot(self.time_steps, self.k_att_history, label='k_att', color='blue', marker='o')

    # Plot k_rep over time
        self.ax11.plot(self.time_steps, self.k_rep_history, label='k_rep', color='red', marker='s')

    # Plot d0 over time
        self.ax11.plot(self.time_steps, self.d0_history, label='d0', color='green', marker='^')

    # Add labels and legend
        self.ax11.set_xlabel('Time Step')
        self.ax11.set_ylabel('Parameter Value')
        self.ax11.set_title('Parameter Values Over Time')
        self.ax11.legend()
    ...
    def stopping(self):
        ...
        if end_distance < 0.05:
            ...
            # self.fig.savefig(f"plots/mppi/varying_sampled/mppi_varying_sampled_{run_number}_success.png")
...
def get_current_run_number():
    try:
        with open("plots/mppi/varying_sampled/run_results_mppi_varying_sampled.txt", "r") as f:
            lines = f.readlines()
        return len(lines) + 1
    except FileNotFoundError:
        return 1
def main(args=None):
    ...
    modified_apf_node= ModifiedAPF()

    try:
        rclpy.spin(modified_apf_node)
    except KeyboardInterrupt:
        print("\n[Ctrl+C] Interrupted — checking run status.")

        if not modified_apf_node.result_saved:
            save_failed_run()
