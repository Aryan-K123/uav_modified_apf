...
class ImageSubscriber(Node):
  ...
  def __init__(self):
    ...
    self.vel_cmd=self.create_publisher(Twist,"/simple_drone/cmd_vel",self.landing_callback,10)
    self.subscription # prevent unused variable warning

    self.landing_sub = self.create_subscription(Bool,"/landing",10)
    self.height_pub = self.create_publisher(Bool,"/height",10)
